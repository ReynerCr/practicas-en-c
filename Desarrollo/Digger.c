...
int juego (int map[F][C], int nivel) {
	...
	while (resp!='x') {
		...
		//CONTADOR SIN NECESIDAD DE FOR, CUANDO LLEGUE A 3 EL BLOQUE CAE; SE UTILIZARíA UNA VARIABLE auxBLOCK PARA QUE SE ACTIVE LA CONDICIÓN DEL IF (auxBLOCK==3) {  if (map[bf][bc]==0) bc++;
		} //DERECHA
		map[DF][DC]=3;
		imprimir (map, nivel);
		printf ("\n\n$=%d\t@=%d", contdin, contv);
